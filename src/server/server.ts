...
import type { Octotask } from "../index.js";
...
export class Server {
  ...
  public octotaskApp: Octotask;

  private state: State;

  constructor(options: ServerOptions = {} as ServerOptions) {
    this.expressApp = express();
    this.octotaskApp = new options.Octotask({
      request: options.request,
    });
    this.log = options.log
      ? rebindLog(options.log)
      : rebindLog(this.octotaskApp.log.child({ name: "server" }));
    ...
    this.expressApp.use(
      createWebhooksMiddleware(this.octotaskApp.webhooks, {
  ...
  public async start() {
    this.log.info(
      `Running Octotask v${this.version} (Node.js: ${process.version})`,
    );
