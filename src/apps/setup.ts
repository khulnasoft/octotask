...
export const setupAppFactory = (
  ...
  async function setupApp(
    ...
    route.get("/", (_req, res: ServerResponse) =>
      res
        .writeHead(302, { "content-type": "text/plain", location: `/octotask` })
        .end(`Found. Redirecting to /octotask`),
    );
  };
...
function printRestartMessage(app: Octotask) {
  app.log.info("");
  app.log.info("Octotask has been set up, please restart the server!");
  app.log.info("");
}
