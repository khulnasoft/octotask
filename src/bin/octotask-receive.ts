// Usage: octotask receive -e push -p path/to/payload app.js
...
import { Octotask, type ApplicationFunctionOptions } from "../index.js";
...
async function main() {
  ...
  await octotask.load(appFn, options);

  octotask.log.debug("Receiving event", event);
  octotask.receive({ name: event, payload, id: uuidv4() }).catch(() => {
